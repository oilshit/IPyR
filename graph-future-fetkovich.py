...
fetkovich_data = production_data.data[0]

# Acquire several properties for production
j_present = round(production_data.calculate_present_pi(fetkovich_data), 2)
q_max = round(production_data.calculate_q_max(reservoir_pressure, fetkovich_data), 2)

j_future = round(production_data.calculate_future_pi(fetkovich_data), 2)
future_p_res = production_data.future_p_res

future_q = round(production_data.calculate_future_q(fetkovich_data), 2)
future_data = {
    "q": future_q,
...
fetkovich_graph = production_data.get_production_graph(q_max, iter, reservoir_pressure, fetkovich_data)

flowrate_x = [data["q"] for data in fetkovich_graph]
pressure_y = [data["p"] for data in fetkovich_graph]

plt.plot(flowrate_x, pressure_y, linestyle="dashed", linewidth=.75)
plt.scatter(flowrate_x, pressure_y, label="Fetkovich (current)")
...
fetkovich_future_graph = production_data.get_production_graph(future_q_max, iter, future_p_res, future_data)

flowrate_x_1 = [data["q"] for data in fetkovich_future_graph]
pressure_y_1 = [data["p"] for data in fetkovich_future_graph]

plt.plot(flowrate_x_1, pressure_y_1, linestyle="dashed", linewidth=.75)
plt.scatter(flowrate_x_1, pressure_y_1, label="Fetkovich (3 years later)")
